...
function formatCountdown(ms) {
  if (ms <= 0) return null; // promo ended
...
function App() {
  ...
  // Shared state (payload, credits, find, etc.) now lives in AppContext so
  // SearchList — which is its own route — can read/write the same data.
  const {
    loader, find, setfind, username, setsearching, bar,
  } = useAppContext();

  // ── page-local state (footer suggestion box, countdown) ──
  ...
  useEffect(() => {
    if (!countdown) return; // promo already ended on mount

    const id = setInterval(() => {
      const remaining = COUNTDOWN_DATE - Date.now();
  ...
  useEffect(() => {
    const parts = `${new Date()}`.split(" ");
    setdated(`${parts[2]} ${parts[1]} ${parts[3]}`);
  }, []);
    ...
    <div className="page">
      ...
      <div className="landingpage">
        <ul className="mlist">
          ...
          <div className="rightmenu">
            ...
            <div className="space" />
            <div className="onne" />
            <div className="onne" title={username || "GUEST"}>
              <div className="dp">
                {username ? username.toUpperCase()[0] : "Go-pro"}
              </div>
            </div>
          </div>
        </ul>

        {/* sparkle SVG — only shown when not loading */}
        <svg
      ...
      <div className="slideme">
        <div className="slideitem">
          <div className="slideitemhead"><div className="fnav">❔</div>Questions</div>
          <div className="slideitembody">
            <div className="slideb1"><p>Click the 🔍☝️search box and type your course code</p></div>
            <div className="slideb2"><img width="110" src={inspo2} alt="Past Questions" /></div>
          </div>
        </div>
        <div className="slideitem">
          <div className="slideitemhead"><div className="fnav">♟️</div>Strategy</div>
          <div className="slideitembody">
            <div className="slideb1"><p>Increased retention with psychological learning concepts</p></div>
            <div className="slideb2"><img width="110" src={inspo} alt="Strategy" /></div>
          </div>
        </div>
        <div className="slideitem">
          <div className="slideitemhead"><div className="fnav">📖</div>Revision</div>
          <div className="slideitembody">
            <div className="slideb1"><p>Practice with our browser extension</p></div>
            <div className="slideb2"><img width="110" src={inspo1} alt="Revision" /></div>
          </div>
        </div>
      </div>
      ...
      <LoadComponent opacity={loader ? 1 : 0} indexed={loader ? 100 : -100} mainlogo={mainlogo} />

      <div className="scrldn"><div className="scrd" /></div>

      {/* ── footer ── */}
      <div className="footer">
        <div className="foot1">
          <img className="brands" title="uelearn"         src={mainlogo}  alt="UELearn" />
          <img className="brands" title="unityelites.com" src={uelogo}     alt="Unity Elites" />
        ...
        <div className="foot1">
          <div className="twwo" title="Make a suggestion">
            <div className="search">
              {spin
                ? <img src={spinner} className="spinner" width={200} alt="Sending…" />
                : <span className="fnav" style={{ width: 30, margin: 5, height: 30 }}>
                    <i className="fa fa-cloud" style={{ fontSize: 10, color: "whitesmoke" }} />
                  </span>
              }
            </div>
            <div className="input">
