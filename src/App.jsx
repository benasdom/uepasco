...
useEffect(() => {
 ...
 stored?.length?setshows(false):false;
...
<div className="rightmenu">
    <Search setsearching={setsearching} eprop={"none"} setfind={setfind} find={find} searching={searching} bar={bar}/>
                ...
                <div className="onne" title={username}><div className="dp">{username.length?username.toUpperCase()[0]:"Go-pro"}</div></div>
            </div>  
                      </ul>
...
<div className="welcome">🤗 Sup <span className="welcome" style={{color:"black"}}>{username}</span> !<span className="prem4 difficon">👋🏾</span>
</div>
